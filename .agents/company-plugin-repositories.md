...
下表列出仓库名称，按实际检出位置定位；公司插件源码位于 GitLab 的 `chenyonghao` 命名空间，GitHub 镜像归属为 `CleverC2200`。实施前核对实际 remote、分支和未提交改动，不凭目录名判断归属。
...
涉及公司 DSH 插件发版、GitLab 发行库或桌面更新频道时，使用 `dsh-plugin-release` 技能；它是公司版发布入口，旧 GitHub/npm 流程不自动适用于公司版。普通开发继续遵循本仓库规则。

官方 DSH 核心发版不使用这条公司插件路线。
